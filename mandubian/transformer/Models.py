...
class UTDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.dec = DecoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
    ...
    def forward(self, tgt_seq, tgt_pos, src_seq, enc_output, return_attns=False):
        ...
        if(self.act):
            dec_output, (remainders,n_updates) = self.act_fn(dec_output, tgt_seq, self.dec, tgt_pos, self.position_signal, self.num_layers, encoder_output)
        for dec_layer in self.layer_stack:
            dec_output, dec_slf_attn, dec_enc_attn = dec_layer(
                dec_output, enc_output,
                non_pad_mask=non_pad_mask,
