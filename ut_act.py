...
while True:
    ...
    for __ in range(GRADIENT_ACCUMULATE_EVERY):
        ...
        # print(batch_as)
        pred_as, act_losses = model(batch_qs, batch_as, return_emb=False)
        R_t = act_losses[0]
        N_t = act_losses[1]
        p_t = R_t + N_t

        # average act loss
        avg_p_t = torch.mean(p_t, [0,1])
        ...
        # total act loss
        train_loss += float(ACT_WEIGHT) * avg_p_t
        ...
        del train_loss, avg_p_t, act_losses, R_t, N_t, p_t
...
#     if i % GRADIENT_ACCUMULATE_EVERY == 0:
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            ...
            # forward
            pred_as, act_losses = model(batch_qs, batch_as,)
