...
import numpy as np
import math
from ut_pytorch.models.common_layer import EncoderLayer, DecoderLayer, MultiHeadAttention, Conv, PositionwiseFeedForward, LayerNorm, _gen_bias_mask, _gen_timing_signal
...
class UTransformer(nn.Module):
    ...
    Outputs will have the shape [batch_size, ]
    """
    def __init__(self, num_vocab, embedding_size=512, hidden_size=512, num_layers=6, num_heads=8, total_key_depth=64, total_value_depth=64,
                 filter_size=2048, enc_seq_max_length=71, dec_seq_max_length=32, input_dropout=0.0, layer_dropout=0.0, 
                 attention_dropout=0.0, relu_dropout=0.0, use_mask=False, act=False):
        super().__init__()
        self.embedding_dim = embedding_size
        self.emb = nn.Embedding(num_vocab, embedding_size, padding_idx=0)
        ...
        # sharing decoder embedding and decoder tgt projection weights requires scaling factor
        self.x_logit_scale = (hidden_size ** -0.5)

        # Share the weight matrix between target word embedding & the final logit dense layer
        self.W.weight = self.emb.weight
        
    def forward(self, question, answer,return_emb=False):

        # leave out the ending token. so that dimensions match
        answer = answer[:, :-1]

        ## use same embedding for src and target
        embed_question = self.emb(question)
        embed_answer = self.emb(answer)

        ## APPLY TRANSFORMER
        enc_output, act = self.encoder(embed_question)
        dec_output = self.decoder(embed_answer, encoder_output=enc_output)
        
        # decoder output uses the same weights as tgt emb
        predictions = self.W(dec_output) * self.x_logit_scale

        return predictions, act
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, src_pos=None, return_attns=None):
        del src_pos, return_attns
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, tgt_pos=None, src_seq=None, encoder_output=None, return_attns=None):
        del tgt_pos, src_seq, return_attns
        #Add input dropout
        x = self.input_dropout(inputs)
